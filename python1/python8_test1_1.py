...
with open('Python_08.codons-6frames.nt.txt', 'w') as fo:
    for name_seq in parsed:
        for n in range(3):
            ...
            parsed[name_seq]['codons +'][f'frame{n+1}']['codons']= codons
            fo.write(f'seq{name_seq}-strand + frame{n+1}-codons\n')
            for codon in codons:
                fo.write(f'{codon} ')
            fo.write('\n')
            ...
            if n not in parsed[name_seq]['codons -']:
                ...
                parsed[name_seq]['codons -'][f'frame{-1-n}']['codons']={}
            parsed[name_seq]['codons -'][f'frame{-1-n}']['codons']= codonsrev
            fo.write(f'seq{name_seq}-strand - frame{n+1}-codons\n')
...
with open('Python_08.translated.aa.txt', 'w') as fo:
    for name_Seq in parsed:
        ...
        for frame in parsed[name_Seq]['codons +']:
            parsed[name_Seq]['codons +'][frame]['aa'] = ''
            fo.write(f'>{frame}:\n')
            for i in parsed[name_Seq]['codons +'][frame]['codons']:
                j = translation_table_aa[i]
                parsed[name_Seq]['codons +'][frame]['aa'] = parsed[name_Seq]['codons +'][frame]['aa'] + j
                fo.write(f'{j}')
            fo.write('\n')
        for frame in parsed[name_Seq]['codons -']:
            parsed[name_Seq]['codons -'][frame]['aa'] = ''
...
#Find the longest ORF (M and stop inclided) from among all six translated reading frames for each input sequence
all_orfs_fw = []
all_orfs_rv = []

with open('Python_08.translated-longest.aa.txt', 'w') as fo:
    for name_seq in parsed:
        fo.write(f'>>{name_seq}\n')
        for frame in parsed[name_seq]['codons +']:
            fo.write(f'{frame}\n')
            for item in re.finditer(r'(M[^M]*?)(\*)',parsed[name_seq]['codons +'][frame]['aa']):
                orf = item.group(0)
                fo.write(f'{orf}\n')
                if len(orf) != 0:
                    all_orfs_fw.append(orf)
            sorted_orf = sorted(all_orfs_fw, key= len, reverse=True)
            all_orfs_fw = []
            if len(sorted_orf) != 0:
                fo.write(f'Longest ORF: {sorted_orf[0]}')
            else:
                fo.write('No orf found')
            fo.write('\n\n')
        for frame in parsed[name_seq]['codons -']:
            fo.write(f'{frame}\n')
            for item in re.finditer(r'(M[^M]*?)(\*)',parsed[name_seq]['codons -'][frame]['aa']):
                ...
                fo.write(f'{orf}\n')
                if len(orf) != 0:
                    all_orfs_rv.append(orf)
            sorted_orf = sorted(all_orfs_rv, key= len, reverse=True)
            all_orfs_rv = []
            if len(sorted_orf) != 0:
                fo.write(f'Longest ORF: {sorted_orf[0]}')
            else:
